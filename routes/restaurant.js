        ...
        tokenVerify.verify(con, token, function (check) {
            if (check.status == 'success') {
                ...
                con.query(sql, function (err, rows) {
                    ...
                    if (rows.length > 0) {
                        var hotelID = rows[0].ID;
                        let sql = mysql.format("SELECT name, place, openingTime, closingTime, phone, latitude, longitude, street, city, pincode, body from Hotel, Hotel_Profile p, Hotel_Description d WHERE ID = ? AND ID = p.hotelID AND ID = d.hotelID", [hotelID]);
                        con.query(sql, function (err, rows) {
                            if (err) {
                                throw err;
                            }
